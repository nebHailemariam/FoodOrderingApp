...
from users.permission import IsDeliveryman
from users.models import User
from users.serializers.deliveryman.profile_serializers import DeliverymanProfileSerializer
...
def deliveryman_profile(request):
    if request.method == "GET":
        serializer = DeliverymanProfileSerializer(request.user)
        return Response(serializer.data, status.HTTP_200_OK)

    elif request.method == "POST":
        serializer = DeliverymanProfileSerializer(data=request.data["user"])
